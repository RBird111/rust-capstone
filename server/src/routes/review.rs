...
#[post("")]
async fn create_new_review(state: DBPool, data: web::Json<ReviewForm>) -> Result<impl Responder> {
    let review_data = data.into_inner();

    let review = web::block(move || {
        let mut conn = state.get().expect("error connecting to database");
        review::create_new_review(&mut conn, review_data)
    })
    .await?
    .map_err(ErrorNotFound)?;
...
// #[get("/user/curr")]
// async fn get_user_reviews(state: DBPool) -> Result<Vec<Review>> {
//     todo!()
// }
